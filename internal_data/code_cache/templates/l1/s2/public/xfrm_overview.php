<?php
// FROM HASH: b8339a081df8af82cd8d638a93b67ad4
return array(
'code' => function($__templater, array $__vars, $__extensions = null)
{
	$__finalCompiled = '';
	$__templater->pageParams['pageTitle'] = $__templater->preEscaped('xfrm_resources');
...
';
	$__templater->setPageParam('searchConstraints', array('xfrm_resources' => array('search_type' => 'resource', ), ));
	$__finalCompiled .= '

';
	$__templater->pageParams['pageAction'] = $__templater->preEscaped('
	' . $__templater->button('xfrm_add_resource...', array(
...
<div class="block" data-xf-init="' . ($__vars['canInlineMod'] ? 'inline-mod' : '') . '" data-type="resource" data-href="' . $__templater->func('link', array('inline-mod', ), true) . '">
	...
	if (!$__templater->test($__vars['resources'], 'empty', array())) {
		...
		$__finalCompiled .= '
				<div class="block-row">' . 'xfrm_there_no_resources_matching_your_filters' . '</div>
			';
	} else {
		$__finalCompiled .= '
				<div class="block-row">' . 'xfrm_no_resources_have_been_created_yet' . '</div>
			';
	}
	$__finalCompiled .= '
		</div>
